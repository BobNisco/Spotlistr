          ...
          doReplaceRequest(
            ...
            function() {
              ...
              _this.addTracks(
                ...
                playlist_id,
                access_token,
                playlist.slice(_this.SPOTIFY_TRACK_LIMIT),
                successCallback,
                errorCallback
              );
            },
      ...
      getVideosFromPlaylist: function(playlistId, results, nextPageToken, callback) {
        ...
        $http.get(req).success(function(res) {
          for (var i = 0; i < res.items.length; i += 1) {
            if (res.items[i].snippet.title !== "This video has been deleted" && res.items[i].snippet.title !== "Private video") {
